...
class TestOrganizationUnitsController(unittest.TestCase):
    def test_should_delete_organization_unit(self):
        organization_unit_app_service_mock = Mock()
        organization_unit_app_service_mock.delete_async.return_value = OrganizationUnitOutput(
            id=str(uuid.uuid4()),
            selected_users=[get_test_user_output("test_user_for_delete_ou")],
            selected_roles=[get_test_role_output("test_role_for_delete_ou")]
        )
        
        organization_units_controller = OrganizationUnitsController(organization_unit_app_service_mock)
        action_result = organization_units_controller.delete_organization_units(str(uuid.uuid4()))
