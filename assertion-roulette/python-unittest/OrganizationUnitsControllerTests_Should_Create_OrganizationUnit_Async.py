...
class TestOrganizationUnitsController(unittest.TestCase):
    def test_should_create_organization_unit(self):
        organization_unit_app_service_mock = Mock()
        organization_unit_app_service_mock.create.return_value = OrganizationUnitOutput(
            id=str(uuid.uuid4()),
            selected_users=[get_test_user_output("test_user_for_insert_ou")],
            selected_roles=[get_test_role_output("test_role_for_insert_ou")]
        )
        
        organization_units_controller = OrganizationUnitsController(organization_unit_app_service_mock)
        action_result = organization_units_controller.post_organization_units(CreateOrganizationUnitInput())
