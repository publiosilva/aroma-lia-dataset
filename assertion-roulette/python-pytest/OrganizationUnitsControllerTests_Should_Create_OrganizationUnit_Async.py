...
@pytest.mark.asyncio
async def test_should_create_organization_unit_async():
    organization_unit_app_service_mock = AsyncMock()
    organization_unit_app_service_mock.create_async.return_value = {
        "Id": "some-guid",
        "SelectedUsers": [{"Username": "test_user_for_insert_ou"}],
        "SelectedRoles": [{"RoleName": "test_role_for_insert_ou"}]
    }
    organization_units_controller = OrganizationUnitsController(organization_unit_app_service_mock)
    action_result = await organization_units_controller.post_organization_units({})
