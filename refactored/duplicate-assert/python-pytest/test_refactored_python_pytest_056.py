...
class TestResourceTrackerService:
    ...
    def test_decommission_with_exclude_hosts_1(self):
        conf = Configuration()
    ...
    def test_decommission_with_exclude_hosts_2(self):
        conf = Configuration()
        conf.set(YarnConfiguration.RM_NODES_EXCLUDE_FILE_PATH, hostFile.getAbsolutePath())
    ...
    def test_decommission_with_exclude_hosts_3(self):
        conf = Configuration()
        conf.set(YarnConfiguration.RM_NODES_EXCLUDE_FILE_PATH, hostFile.getAbsolutePath())
    ...
    def test_decommission_with_exclude_hosts_4(self):
        conf = Configuration()
        conf.set(YarnConfiguration.RM_NODES_EXCLUDE_FILE_PATH, hostFile.getAbsolutePath())
        ...
        assert NodeAction.SHUTDOWN == node_heartbeat.get_node_action(), "The decommissioned metrics are not updated"
        check_decommissioned_nm_count(rm, metric_count + 2)
