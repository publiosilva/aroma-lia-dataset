...
class TestFsActionExecutor:
    ...
    def test_do_operations_with_name_node_element_1(self):
        ae = FsActionExecutor()
        fs = get_file_system()

        mkdir = Path(get_fs_test_case_dir(), "mkdir")
        mkdir_x = Path(mkdir.to_uri().get_path())
        delete = Path(get_fs_test_case_dir(), "delete")
        delete_x = Path(delete.to_uri().get_path())
        fs.mkdirs(delete)
        source = Path(get_fs_test_case_dir(), "source")
        source_x = Path(source.to_uri().get_path())
        fs.mkdirs(source)
        target = Path(Path(get_fs_test_case_dir(), "target").to_uri().get_path())
        chmod1 = Path(get_fs_test_case_dir(), "chmod1")
        chmod1_x = Path(chmod1.to_uri().get_path())
        fs.mkdirs(chmod1)
        child1 = Path(chmod1, "child1")
        fs.mkdirs(child1)
        chmod2 = Path(get_fs_test_case_dir(), "chmod2")
        chmod2_x = Path(chmod2.to_uri().get_path())
        fs.mkdirs(chmod2)
        child2 = Path(chmod2, "child2")
        fs.mkdirs(child2)
        new_file1 = Path(mkdir + "newFile1")
        new_file1_x = Path(new_file1.to_uri().get_path())
        new_file2 = Path(mkdir + "newFile2")
        new_file2_x = Path(new_file2.to_uri().get_path())
        fs.create_new_file(new_file1)
        chmod3 = Path(get_fs_test_case_dir(), "chmod3")
        chmod3_x = Path(chmod3.to_uri().get_path())
        fs.mkdirs(chmod3)
        ...
        str = "<root><name-node>{0}</name-node>" \
              "<mkdir path=''{1}''/>" \
              "<delete path=''{2}''/>" \
    ...
    def test_do_operations_with_name_node_element_2(self):
        ae = FsActionExecutor()
        fs = get_file_system()
